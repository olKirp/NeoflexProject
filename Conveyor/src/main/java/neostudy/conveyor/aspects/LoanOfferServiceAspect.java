...
@Aspect
@Configuration
public class LoanOfferServiceAspect {

    @Around("execution(* neostudy.conveyor.service.LoanOffersService+.createLoanOffers(..))")
    ...
    @Around("execution(* neostudy.conveyor.service.LoanOffersService+.calculateRate(..))")
    public Object logPrescoringService(ProceedingJoinPoint point) throws Throwable {
        log.info("Calculate rate in LoanOfferService. IsSalaryClient=" + point.getArgs()[0] + ", IsInsurance=" + point.getArgs()[1] + ", base rate=" + point.getArgs()[2]);
